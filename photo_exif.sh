#!/bin/bash 
#A command-line tool for adding borders with exif information and watermarks, 
...
sh tmp.sh 
rm tmp.sh mask.jpg trans.png watermark.png wk.jpg

bottom_font_pt=`echo "$fact_bottom_font_width * $converted_pic_width * 72.0 / 96.0"|bc|cut -d"." -f1`
...
date_X=`echo "$fact_date_place * $converted_pic_width"|bc|cut -d"." -f1`
...
author_write_X_place=`echo "$author_X - $width_of_left_right_bottom - $white_line_width"|bc|cut -d"." -f1`
...
$CVMD +$info_write_X_place+$info_write_Y_place "$Machine $Aperture ${Shutter_speed}s ISO$ISO Ev $Exposure_bias" tmp.jpg tmp.jpg 
